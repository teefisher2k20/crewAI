...
class DrawerManager {
        ...
        <ul class="drawer-list">
          ${grouped.map((group) => {
            if (group.items.length === 1) {
              ...
              return `
                <li>
                  <button type="button" class="trigger-group" data-trigger-items="${groupId}" style="border-left: 2px solid var(--text-secondary); padding: 4px 0 4px 8px; margin: 2px 0; border-radius: 3px; cursor: pointer; transition: background 0.15s ease; width: 100%; text-align: left; background: none; border-top: none; border-right: none; border-bottom: none;">
      ...
      if (stringChildren.length > 0) {
        ...
        children += grouped.map((group) => {
          if (group.items.length === 1) {
            ...
            return `
              <button type="button" class="trigger-group" data-trigger-items="${groupId}" style="border-left: 2px solid ${groupColor}; padding: 8px 0 8px 12px; margin: 4px 0; border-radius: 4px; cursor: pointer; transition: background 0.2s ease; width: 100%; text-align: left; background: none; border-top: none; border-right: none; border-bottom: none;" onmouseover="this.style.background='${groupHoverBg}'" onmouseout="this.style.background='transparent'">
                <span class="trigger-group-label" style="display: inline-flex; color: ${groupColor}; font-size: 11px; font-weight: 600; margin-bottom: 4px; text-transform: uppercase; letter-spacing: 0.5px; background: ${groupBgColor}; padding: 3px 8px; border-radius: 3px; align-items: center; gap: 4px; user-select: none;">
  ...
  renderMetadata(metadata) {
    ...
    if (metadata.router_events && metadata.router_events.length > 0) {
      ...
      metadataContent += `
                <div class="drawer-section">
                    <button type="button" class="drawer-section-title router-events-title" data-router-events="${routerEventsJson}" style="cursor: pointer; display: inline-flex; align-items: center; gap: 4px; background: none; border: none; padding: 0; width: 100%; text-align: left;">
  ...
  renderSourceCode(metadata) {
    ...
    if (metadata.class_signature) {
      ...
      classSection = `
                <div class="accordion-section">
                    <button type="button" class="accordion-subheader accordion-title" aria-expanded="false" aria-controls="class-content" style="width: 100%; text-align: left; background: none; border: none; padding: 10px;">Class</button>
    ...
    return `
            <div class="accordion-section expanded">
                <button type="button" class="accordion-header" aria-expanded="true" aria-controls="source-code-content" style="width: 100%; text-align: left; background: none; border: none; padding: 10px;">
                    <span class="accordion-title">Source Code</span>
