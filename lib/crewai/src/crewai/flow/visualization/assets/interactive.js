...
class DrawerManager {
        ...
        <ul class="drawer-list">
          ${grouped.map((group) => {
            if (group.items.length === 1) {
              return `<li><button class="drawer-code-link" type="button" data-node-id="${group.items[0]}" aria-label="Highlight ${group.items[0]}">${group.items[0]}</button></li>`;
            } else {
              const groupId = group.items.join(',');
              return `
                <li>
                  <button class="trigger-group" type="button" data-trigger-items="${groupId}" aria-label="Highlight ${group.items.length} routes" style="border: none; border-left: 2px solid var(--text-secondary); padding: 4px 0 4px 8px; margin: 2px 0; border-radius: 3px; cursor: pointer; transition: background 0.15s ease; width: 100%; text-align: left; background: none; font: inherit;">
                    <span class="trigger-group-label" style="display: block; font-size: 11px; font-weight: 600; color: var(--text-secondary); margin-bottom: 4px; user-select: none;">
                      ${group.items.length} routes <span style="opacity: 0.5; font-size: 9px;">→</span>
                    </span>
                    <span class="trigger-group-items" style="display: flex; flex-wrap: wrap; gap: 4px; pointer-events: none;">
  ...
  renderConditionTree(condition, depth = 0) {
    if (typeof condition === "string") {
      return `<button class="drawer-code-link trigger-leaf" type="button" data-node-id="${condition}" aria-label="Highlight ${condition}">${condition}</button>`;
    }
    ...
    if (condition.type === "AND" || condition.type === "OR") {
      ...
      if (stringChildren.length > 0) {
        ...
        children += grouped.map((group) => {
          if (group.items.length === 1) {
            ...
            return `
              <button class="trigger-group" type="button" data-trigger-items="${groupId}" aria-label="Highlight ${group.items.length} routes" style="border: none; border-left: 2px solid ${groupColor}; padding: 8px 0 8px 12px; margin: 4px 0; border-radius: 4px; cursor: pointer; transition: background 0.2s ease; width: 100%; text-align: left; background: none; font: inherit;" onmouseover="this.style.background='${groupHoverBg}'" onmouseout="this.style.background='transparent'">
                <span class="trigger-group-label" style="color: ${groupColor}; font-size: 11px; font-weight: 600; margin-bottom: 4px; text-transform: uppercase; letter-spacing: 0.5px; background: ${groupBgColor}; padding: 3px 8px; border-radius: 3px; display: inline-flex; align-items: center; gap: 4px; user-select: none;">
                  ${group.items.length} routes <i data-lucide="chevron-down" style="width: 14px; height: 14px; color: ${groupColor};"></i>
                </span>
                <span class="trigger-group-items" style="display: flex; flex-wrap: wrap; gap: 4px; margin-top: 4px; pointer-events: none;">
      ...
      return `
        <div class="condition-group" data-trigger-group="${triggerIdsJson}" style="border-left: 2px solid ${color}; padding: 8px 0 8px 12px; margin: 4px 0; transition: background 0.2s ease; position: relative; border-radius: 4px;" onmouseover="this.style.background='${hoverBg}'" onmouseout="this.style.background='transparent'">
          <button class="condition-label" type="button" data-condition-label="${conditionType}" aria-label="Highlight ${conditionType} condition group" style="color: ${color}; font-size: 11px; font-weight: 600; margin-bottom: 4px; text-transform: uppercase; letter-spacing: 0.5px; background: ${bgColor}; padding: 3px 8px; border-radius: 3px; display: inline-flex; align-items: center; gap: 4px; cursor: pointer; user-select: none; border: none; font: inherit;">
  ...
  renderMetadata(metadata) {
    ...
    if (metadata.router_events && metadata.router_events.length > 0) {
      ...
      metadataContent += `
                <div class="drawer-section">
                    <button type="button" class="drawer-section-title router-events-title" data-router-events="${routerEventsJson}" aria-label="Highlight all router events" style="cursor: pointer; display: inline-flex; align-items: center; gap: 4px; background: none; border: none; padding: 0; font: inherit; text-align: left; width: 100%;">
                        Router Events <i data-lucide="chevron-down" style="width: 14px; height: 14px; color: var(--text-primary);"></i>
                    </button>
                    <ul class="drawer-list">
                        ${uniqueRouterEvents.map((eventName) => `<li><button class="drawer-code-link" type="button" data-router-event="${eventName}" aria-label="Highlight ${eventName} event" style="color: {{ CREWAI_ORANGE }}; border: none; border-bottom: 1px solid rgba(255,90,80,0.3); background: none; padding: 4px 8px; font: inherit; font-family: monospace;">${eventName}</button></li>`).join("")}
                    </ul>
                </div>
            `;
    }
