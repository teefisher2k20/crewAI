...
class DrawerManager {
        ...
        <ul class="drawer-list">
          ${grouped.map((group) => {
            if (group.items.length === 1) {
              ...
              return `
                <li>
                  <button type="button" class="trigger-group" data-trigger-items="${groupId}" style="border-left: 2px solid var(--text-secondary); padding: 4px 0 4px 8px; margin: 2px 0; border-radius: 3px; cursor: pointer; transition: background 0.15s ease;">
      ...
      if (stringChildren.length > 0) {
        ...
        children += grouped.map((group) => {
          if (group.items.length === 1) {
            ...
            return `
              <button type="button" class="trigger-group" data-trigger-items="${groupId}" style="border-left: 2px solid ${groupColor}; padding: 8px 0 8px 12px; margin: 4px 0; border-radius: 4px; cursor: pointer; transition: background 0.2s ease;" onmouseover="this.style.background='${groupHoverBg}'" onmouseout="this.style.background='transparent'">
    ...
    if (metadata.router_paths && metadata.router_paths.length > 0) {
      ...
      metadataContent += `
                <div class="drawer-section">
                    ...
                    <ul class="drawer-list">
                        ${uniqueRouterPaths.map((p) => `<li><button type="button" class="drawer-code-link" data-node-id="${p}" style="color: {{ CREWAI_ORANGE }}; border-color: rgba(255,90,80,0.3);">${p}</button></li>`).join("")}
                    </ul>
                </div>
            `;
    }
  ...
  animateOpen() {
    ...
    if (!wasAlreadyOpen) {
      ...
      // Move focus for accessibility when switching content in an already open drawer
      this.elements.closeButton.focus();
    }
  }
  ...
  close() {
    ...
    }, CONSTANTS.ANIMATION.DURATION + 50);
  }
