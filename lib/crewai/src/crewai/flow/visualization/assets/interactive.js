...
class DrawerManager {
  ...
  renderMetadata(metadata) {
    ...
    if (metadata.router_paths && metadata.router_paths.length > 0) {
      ...
      metadataContent += `
                <div class="drawer-section">
                    <button type="button" class="drawer-section-title router-paths-title" data-router-paths="${routerPathsJson}" aria-expanded="false" aria-controls="router-paths-list" style="cursor: pointer; display: inline-flex; align-items: center; gap: 4px;">
                        Router Paths <i data-lucide="chevron-down" style="width: 14px; height: 14px; color: var(--text-primary);"></i>
                    </button>
                    <ul class="drawer-list" id="router-paths-list">
    ...
    if (metadata.class_signature) {
      ...
      classSection = `
                <div>
                    ...
                    <div class="drawer-section" id="class-content">
  ...
  attachContentEventListeners(nodeName) {
    ...
    if (routerPathsTitle) {
      routerPathsTitle.addEventListener("click", (e) => {
        e.preventDefault();
        e.stopPropagation();
        const isExpanded = routerPathsTitle.getAttribute('aria-expanded') === 'true';
        routerPathsTitle.setAttribute('aria-expanded', !isExpanded);
...
class NetworkManager {
  ...
  setupTheme() {
    ...
    const updateThemeIcon = (isDark) => {
      ...
      const themeTitle = isDark ? "Toggle Light Mode" : "Toggle Dark Mode";
