...
class DrawerManager {
  ...
  renderMetadata(metadata) {
    ...
    if (metadata.router_paths && metadata.router_paths.length > 0) {
      ...
      metadataContent += `
                <div class="drawer-section">
                    <button type="button" class="drawer-section-title router-paths-title" data-router-paths="${routerPathsJson}" style="display: inline-flex; align-items: center; gap: 4px;">
  ...
  renderSourceCode(metadata) {
    ...
    if (metadata.class_signature) {
      ...
      classSection = `
                <div>
                    <button type="button" class="accordion-subheader accordion-title" aria-controls="class-content" aria-expanded="true" style="width: 100%; text-align: left; border: none; font-family: inherit; cursor: pointer;">Class</button>
