...
class DrawerManager {
        ...
        <ul class="drawer-list">
          ${grouped.map((group) => {
            if (group.items.length === 1) {
              return `<li><button type="button" class="drawer-code-link" data-node-id="${group.items[0]}">${group.items[0]}</button></li>`;
            } else {
              const groupId = group.items.join(',');
              return `
                <li>
                  <button type="button" class="trigger-group" data-trigger-items="${groupId}" style="border-left: 2px solid var(--text-secondary); padding: 4px 0 4px 8px; margin: 2px 0; border-radius: 3px; cursor: pointer; transition: background 0.15s ease; border-right: none; border-top: none; border-bottom: none; background: none; width: 100%; text-align: left; display: block; font-family: inherit;">
  ...
  renderConditionTree(condition, depth = 0) {
    if (typeof condition === "string") {
      return `<button type="button" class="drawer-code-link trigger-leaf" data-node-id="${condition}">${condition}</button>`;
    }
    ...
    if (condition.type === "AND" || condition.type === "OR") {
      ...
      if (stringChildren.length > 0) {
        ...
        children += grouped.map((group) => {
          if (group.items.length === 1) {
            ...
            return `
              <button type="button" class="trigger-group" data-trigger-items="${groupId}" style="border-left: 2px solid ${groupColor}; padding: 8px 0 8px 12px; margin: 4px 0; border-radius: 4px; cursor: pointer; transition: background 0.2s ease; border-right: none; border-top: none; border-bottom: none; background: none; width: 100%; text-align: left; display: block; font-family: inherit;" onmouseover="this.style.background='${groupHoverBg}'" onmouseout="this.style.background='transparent'">
      ...
      return `
        <div class="condition-group" style="border-left: 2px solid ${color}; padding: 8px 0 8px 12px; margin: 4px 0; transition: background 0.2s ease; position: relative; border-radius: 4px;" onmouseover="this.style.background='${hoverBg}'" onmouseout="this.style.background='transparent'">
          <button type="button" class="condition-label" data-condition-label="${conditionType}" data-trigger-group="${triggerIdsJson}" style="color: ${color}; font-size: 11px; font-weight: 600; margin-bottom: 4px; text-transform: uppercase; letter-spacing: 0.5px; background: ${bgColor}; padding: 3px 8px; border-radius: 3px; display: inline-flex; align-items: center; gap: 4px; cursor: pointer; user-select: none; border: none; font-family: inherit;">
  ...
  renderMetadata(metadata) {
    ...
    if (metadata.router_paths && metadata.router_paths.length > 0) {
      ...
      metadataContent += `
                <div class="drawer-section">
                    <button type="button" class="drawer-section-title router-paths-title" data-router-paths="${routerPathsJson}" style="cursor: pointer; display: inline-flex; align-items: center; gap: 4px; border: none; background: none; padding: 0; font-family: inherit; width: 100%; text-align: left;" aria-label="Highlight router paths">
                        Router Paths <i data-lucide="chevron-down" style="width: 14px; height: 14px; color: var(--text-primary);"></i>
                    </button>
                    <ul class="drawer-list">
                        ${uniqueRouterPaths.map((p) => `<li><button type="button" class="drawer-code-link" data-node-id="${p}" style="color: {{ CREWAI_ORANGE }}; border-color: rgba(255,90,80,0.3); background: none; border: 1px solid rgba(255,90,80,0.3); padding: 4px 8px; font-family: monospace; cursor: pointer; border-radius: 3px;">${p}</button></li>`).join("")}
                    </ul>
                </div>
            `;
    }
  ...
  renderSourceCode(metadata) {
    ...
    if (metadata.class_signature) {
      ...
      classSection = `
                <div class="accordion-section">
                    <button type="button" class="accordion-subheader accordion-title" aria-expanded="false" aria-controls="class-content" style="border: none; width: 100%; font-family: inherit;">Class</button>
                    <div class="accordion-content" id="class-content">
                        <div class="drawer-section">
    ...
    return `
            <div class="accordion-section expanded">
                <button type="button" class="accordion-header" aria-expanded="true" aria-controls="source-code-content" style="border: none; width: 100%; font-family: inherit;">
                ...
                <div class="accordion-content" id="source-code-content">
                    <div class="drawer-section">
                        <div class="code-block-container">
  ...
  attachContentEventListeners(nodeName) {
    ...
    const accordionHeaders =
      this.elements.content.querySelectorAll(".accordion-header, .accordion-subheader");
    accordionHeaders.forEach((header) => {
      header.addEventListener("click", () => {
        const accordionSection = header.closest(".accordion-section");
        const isExpanded = accordionSection.classList.toggle("expanded");
    ...
    const conditionLabels = this.elements.content.querySelectorAll(
      ".condition-label[data-trigger-group]",
    );
    conditionLabels.forEach((label) => {
      label.addEventListener("click", (e) => {
        e.preventDefault();
        e.stopPropagation();
        const triggerIds = JSON.parse(label.getAttribute("data-trigger-group"));
    ...
    if (routerPathsTitle) {
      ...
      routerPathsTitle.addEventListener("mouseover", () => {
        routerPathsTitle.style.background = "rgba(102,102,102,0.05)";
      });
      routerPathsTitle.addEventListener("mouseout", () => {
        routerPathsTitle.style.background = "none";
      });
    }
  }
  ...
  close() {
    ...
    accordions.forEach(accordion => {
      ...
      setTimeout(() => {
        ...
        const header = accordion.querySelector(".accordion-header, .accordion-subheader");
        if (header) {
