...
class DrawerManager {
  ...
  renderConditionTree(condition, depth = 0) {
    ...
    if (condition.type === "AND" || condition.type === "OR") {
          ...
          if (group.items.length === 1) {
            ...
            return `
              <div class="trigger-group" data-trigger-items="${groupId}" style="border-left: 2px solid ${groupColor}; padding: 8px 0 8px 12px; margin: 4px 0; border-radius: 4px; cursor: pointer; transition: background 0.2s ease;" onmouseover="this.style.background='${groupHoverBg}'" onmouseout="this.style.background='transparent'">
                <button type="button" class="trigger-group-label" aria-label="Highlight ${group.items.length} routes" style="color: ${groupColor}; font-size: 11px; font-weight: 600; margin-bottom: 4px; text-transform: uppercase; letter-spacing: 0.5px; background: ${groupBgColor}; padding: 3px 8px; border-radius: 3px; display: inline-flex; align-items: center; gap: 4px; user-select: none; border: none; font-family: inherit; cursor: pointer;">
      ...
      const conditionContentId = `condition-content-${Math.random().toString(36).substr(2, 9)}`;
      return `
        <div class="condition-group" data-trigger-group="${triggerIdsJson}" style="border-left: 2px solid ${color}; padding: 8px 0 8px 12px; margin: 4px 0; transition: background 0.2s ease; position: relative; border-radius: 4px;" onmouseover="this.style.background='${hoverBg}'" onmouseout="this.style.background='transparent'">
          <button type="button" class="condition-label" data-condition-label="${conditionType}" aria-expanded="true" aria-controls="${conditionContentId}" aria-label="Highlight ${conditionType} path" style="color: ${color}; font-size: 11px; font-weight: 600; margin-bottom: 4px; text-transform: uppercase; letter-spacing: 0.5px; background: ${bgColor}; padding: 3px 8px; border-radius: 3px; display: inline-flex; align-items: center; gap: 4px; cursor: pointer; user-select: none; border: none; font-family: inherit;">
            ${conditionType} <i data-lucide="chevron-down" style="width: 14px; height: 14px; color: ${color};"></i>
          </button>
          <div class="condition-children" id="${conditionContentId}" style="margin-top: 4px;">
            ${children}
          </div>
        </div>
      `;
  ...
  renderMetadata(metadata) {
    ...
    if (metadata.router_paths && metadata.router_paths.length > 0) {
      ...
      const routerPathsListId = `router-paths-list-${Math.random().toString(36).substr(2, 9)}`;
      metadataContent += `
                <div class="drawer-section">
                    <button type="button" class="drawer-section-title router-paths-title" data-router-paths="${routerPathsJson}" aria-expanded="true" aria-controls="${routerPathsListId}" aria-label="Highlight all router paths" style="cursor: pointer; display: inline-flex; align-items: center; gap: 4px;">
                        Router Paths <i data-lucide="chevron-down" style="width: 14px; height: 14px; color: var(--text-primary);"></i>
                    </button>
                    <ul class="drawer-list" id="${routerPathsListId}">
  ...
  renderSourceCode(metadata) {
    ...
    if (metadata.class_signature) {
      ...
      const classContentId = `class-content-${Math.random().toString(36).substr(2, 9)}`;
      classSection = `
                <div class="accordion-section expanded">
                    <button type="button" class="accordion-subheader accordion-title" aria-expanded="true" aria-controls="${classContentId}">
                        Class
                        <i data-lucide="chevron-down" class="accordion-icon" style="width: 16px; height: 16px;"></i>
                    </button>
                    <div class="accordion-content" id="${classContentId}">
                        <div class="drawer-section">
    ...
    const sourceCodeContentId = `source-code-content-${Math.random().toString(36).substr(2, 9)}`;
    return `
            <div class="accordion-section expanded">
                <button type="button" class="accordion-header" aria-expanded="true" aria-controls="${sourceCodeContentId}">
                ...
                <div class="accordion-content" id="${sourceCodeContentId}">
                    <div class="drawer-section">
                        <div class="code-block-container">
                            <button type="button" class="code-copy-button" data-code="${codeToHighlight.replace(/"/g, "&quot;")}" aria-label="Copy code">
  ...
  attachContentEventListeners(nodeName) {
    ...
    accordionHeaders.forEach((header) => {
      header.addEventListener("click", () => {
        const accordionSection = header.closest(".accordion-section, .condition-group, .drawer-section");
