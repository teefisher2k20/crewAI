...
class DrawerManager {
  ...
  renderConditionTree(condition, depth = 0) {
    ...
    if (condition.type === "AND" || condition.type === "OR") {
            ...
            return `
              <div role="button" tabindex="0" class="trigger-group" data-trigger-items="${groupId}" style="border-left: 2px solid ${groupColor}; padding: 8px 0 8px 12px; margin: 4px 0; border-radius: 4px; cursor: pointer; transition: background 0.2s ease;" onmouseover="this.style.background='${groupHoverBg}'" onmouseout="this.style.background='transparent'">
                ...
                <div class="trigger-group-items" style="display: flex; flex-wrap: wrap; gap: 4px; margin-top: 4px; pointer-events: none;">
                  ${group.items.map((t) => `<span class="drawer-code" style="opacity: 0.7; cursor: default;">${t}</span>`).join("")}
                </div>
              </div>
            `;
          }
      ...
      return `
        <div role="button" tabindex="0" class="condition-group" data-trigger-group="${triggerIdsJson}" style="border-left: 2px solid ${color}; padding: 8px 0 8px 12px; margin: 4px 0; transition: background 0.2s ease; position: relative; border-radius: 4px;" onmouseover="this.style.background='${hoverBg}'" onmouseout="this.style.background='transparent'">
  ...
  renderMetadata(metadata) {
    ...
    if (metadata.router_paths && metadata.router_paths.length > 0) {
      ...
      metadataContent += `
                <div class="drawer-section">
                    <button type="button" class="drawer-section-title router-paths-title" data-router-paths="${routerPathsJson}" style="cursor: pointer; display: inline-flex; align-items: center; gap: 4px; background: none; border: none; padding: 0; font-family: inherit; width: auto; text-align: left;">
  ...
  renderSourceCode(metadata) {
    ...
    if (metadata.class_signature) {
      ...
      classSection = `
                <div>
                    <button type="button" class="accordion-subheader accordion-title">Class</button>
                    <div class="drawer-section">
