...
class DrawerManager {
        ...
        <ul class="drawer-list">
          ${grouped.map((group) => {
            if (group.items.length === 1) {
              ...
              return `
                <li>
                  <button type="button" class="trigger-group" data-trigger-items="${groupId}" style="border-left: 2px solid var(--text-secondary); padding: 4px 0 4px 8px; margin: 2px 0; border-radius: 3px; cursor: pointer; transition: background 0.15s ease;" aria-label="Highlight ${group.items.length} routes">
                    <div class="trigger-group-label" style="font-size: 11px; font-weight: 600; color: var(--text-secondary); margin-bottom: 4px; user-select: none; text-align: left;">
  ...
  renderMetadata(metadata) {
    ...
    if (metadata.router_events && metadata.router_events.length > 0) {
      ...
      metadataContent += `
                <div class="drawer-section">
                    <button type="button" class="drawer-section-title router-events-title" data-router-events="${routerEventsJson}" style="cursor: pointer; display: inline-flex; align-items: center; gap: 4px;" aria-label="Highlight all router events">
  ...
  renderSourceCode(metadata) {
    ...
    if (metadata.class_signature) {
      ...
      classSection = `
                <div class="accordion-section expanded">
                    <button type="button" class="accordion-subheader accordion-title" aria-expanded="true" aria-controls="class-content">Class</button>
    ...
    return `
            <div class="accordion-section expanded">
                <button type="button" class="accordion-header" aria-expanded="true" aria-controls="source-code-content">
                ...
                <div class="accordion-content" id="source-code-content">
                    <div class="drawer-section">
                        <div class="code-block-container">
                            <button class="code-copy-button" data-code="${codeToHighlight.replace(/"/g, "&quot;")}" aria-label="Copy code" title="Copy code">
  ...
  attachContentEventListeners(nodeName) {
    ...
    const conditionLabels = this.elements.content.querySelectorAll(
      ".condition-label[data-condition-label]",
    );
...
class NetworkManager {
  ...
  setupTheme() {
    ...
    const updateThemeIcon = (isDark) => {
      ...
      const title = isDark ? "Toggle Light Mode" : "Toggle Dark Mode";
