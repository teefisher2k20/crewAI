...
class DrawerManager {
        ...
        <ul class="drawer-list">
          ${grouped.map((group) => {
            if (group.items.length === 1) {
              ...
              return `
                <li>
                  <div class="trigger-group" data-trigger-items="${groupId}" role="button" tabindex="0" aria-label="Highlight routes from ${group.items.join(', ')}" style="border-left: 2px solid var(--text-secondary); padding: 4px 0 4px 8px; margin: 2px 0; border-radius: 3px; cursor: pointer; transition: background 0.15s ease;">
    ...
    if (condition.type === "AND" || condition.type === "OR") {
      ...
      if (stringChildren.length > 0) {
        ...
        children += grouped.map((group) => {
          if (group.items.length === 1) {
            ...
            return `
              <div class="trigger-group" data-trigger-items="${groupId}" role="button" tabindex="0" aria-label="Highlight routes from ${group.items.join(', ')}" style="border-left: 2px solid ${groupColor}; padding: 8px 0 8px 12px; margin: 4px 0; border-radius: 4px; cursor: pointer; transition: background 0.2s ease;" onmouseover="this.style.background='${groupHoverBg}'" onmouseout="this.style.background='transparent'">
      ...
      return `
        <div class="condition-group" data-trigger-group="${triggerIdsJson}" style="border-left: 2px solid ${color}; padding: 8px 0 8px 12px; margin: 4px 0; transition: background 0.2s ease; position: relative; border-radius: 4px;" onmouseover="this.style.background='${hoverBg}'" onmouseout="this.style.background='transparent'">
          <div class="condition-label" data-condition-label="${conditionType}" role="button" tabindex="0" aria-label="Highlight all ${conditionType} routes" style="color: ${color}; font-size: 11px; font-weight: 600; margin-bottom: 4px; text-transform: uppercase; letter-spacing: 0.5px; background: ${bgColor}; padding: 3px 8px; border-radius: 3px; display: inline-flex; align-items: center; gap: 4px; cursor: pointer; user-select: none;">
  ...
  renderMetadata(metadata) {
    ...
    if (metadata.router_paths && metadata.router_paths.length > 0) {
      ...
      metadataContent += `
                <div class="drawer-section">
                    <div class="drawer-section-title router-paths-title" data-router-paths="${routerPathsJson}" role="button" tabindex="0" aria-label="Highlight all router paths" style="cursor: pointer; display: inline-flex; align-items: center; gap: 4px;">
  ...
  attachContentEventListeners(nodeName) {
    ...
    const accessibilityElements = this.elements.content.querySelectorAll(
      ".trigger-group, .condition-label, .router-paths-title"
    );
