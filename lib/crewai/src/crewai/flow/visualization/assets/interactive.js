...
class DrawerManager {
        ...
        <ul class="drawer-list">
          ${grouped.map((group) => {
            if (group.items.length === 1) {
              ...
              return `
                <li>
                  <button type="button" class="trigger-group" data-trigger-items="${groupId}" style="border-left: 2px solid var(--text-secondary); padding: 4px 0 4px 8px; margin: 2px 0; border-radius: 3px; cursor: pointer; transition: background 0.15s ease; border-top: none; border-right: none; border-bottom: none; background: none; width: 100%; text-align: left;">
    ...
    if (condition.type === "AND" || condition.type === "OR") {
      ...
      if (stringChildren.length > 0) {
        ...
        children += grouped.map((group) => {
          if (group.items.length === 1) {
            ...
            return `
              <button type="button" class="trigger-group" data-trigger-items="${groupId}" style="border-left: 2px solid ${groupColor}; padding: 8px 0 8px 12px; margin: 4px 0; border-radius: 4px; cursor: pointer; transition: background 0.2s ease; border-top: none; border-right: none; border-bottom: none; background: none; width: 100%; text-align: left;" onmouseover="this.style.background='${groupHoverBg}'" onmouseout="this.style.background='transparent'">
                ...
                <div class="trigger-group-items" style="display: flex; flex-wrap: wrap; gap: 4px; margin-top: 4px; pointer-events: none;">
                  ${group.items.map((t) => `<span class="drawer-code trigger-leaf" style="opacity: 0.7; cursor: default;">${t}</span>`).join("")}
                </div>
              </button>
            `;
          }
      ...
      return `
        <div class="condition-group" style="border-left: 2px solid ${color}; padding: 8px 0 8px 12px; margin: 4px 0; transition: background 0.2s ease; position: relative; border-radius: 4px;" onmouseover="this.style.background='${hoverBg}'" onmouseout="this.style.background='transparent'">
          <button type="button" class="condition-label" data-trigger-group="${triggerIdsJson}" data-condition-label="${conditionType}" style="color: ${color}; font-size: 11px; font-weight: 600; margin-bottom: 4px; text-transform: uppercase; letter-spacing: 0.5px; background: ${bgColor}; padding: 3px 8px; border-radius: 3px; display: inline-flex; align-items: center; gap: 4px; cursor: pointer; user-select: none; border: none;">
    ...
    if (metadata.router_paths && metadata.router_paths.length > 0) {
      ...
      metadataContent += `
                <div class="drawer-section">
                    ...
                    <ul class="drawer-list">
                        ${uniqueRouterPaths.map((p) => `<li><button type="button" class="drawer-code-link" data-node-id="${p}" style="color: {{ CREWAI_ORANGE }}; border-color: rgba(255,90,80,0.3); background: none; cursor: pointer;">${p}</button></li>`).join("")}
                    </ul>
                </div>
            `;
    }
  ...
  attachContentEventListeners(nodeName) {
    ...
    const conditionLabels = this.elements.content.querySelectorAll(".condition-label[data-trigger-group]");
