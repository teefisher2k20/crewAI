...
class DrawerManager {
    ...
    if (condition.type === "AND" || condition.type === "OR") {
      ...
      if (stringChildren.length > 0) {
        ...
        children += grouped.map((group) => {
          ...
          <button type="button" class="trigger-group-label" style="border: none; font-family: inherit; color: ${groupColor}; font-size: 11px; font-weight: 600; margin-bottom: 4px; text-transform: uppercase; letter-spacing: 0.5px; background: ${groupBgColor}; padding: 3px 8px; border-radius: 3px; display: inline-flex; align-items: center; gap: 4px; user-select: none; width: 100%;">
      ...
      return `
        <div class="condition-group expanded" data-trigger-group="${triggerIdsJson}" style="border-left: 2px solid ${color}; padding: 8px 0 8px 12px; margin: 4px 0; transition: background 0.2s ease; position: relative; border-radius: 4px;" onmouseover="this.style.background='${hoverBg}'" onmouseout="this.style.background='transparent'">
          <button type="button" class="condition-label" data-condition-label="${conditionType}" aria-expanded="true" style="border: none; font-family: inherit; color: ${color}; font-size: 11px; font-weight: 600; margin-bottom: 4px; text-transform: uppercase; letter-spacing: 0.5px; background: ${bgColor}; padding: 3px 8px; border-radius: 3px; display: inline-flex; align-items: center; gap: 4px; cursor: pointer; user-select: none; width: 100%;">
            ${conditionType} <i data-lucide="chevron-down" style="width: 14px; height: 14px; color: ${color};"></i>
          </button>
          <div class="condition-children accordion-content" style="margin-top: 4px;">
            ${children}
          </div>
        </div>
      `;
  ...
  renderMetadata(metadata) {
    ...
    if (metadata.router_paths && metadata.router_paths.length > 0) {
      ...
      metadataContent += `
        <div class="drawer-section accordion-section expanded">
          <button type="button" class="drawer-section-title router-paths-title" data-router-paths="${routerPathsJson}" aria-expanded="true" aria-controls="router-paths-content" style="border: none; background: none; padding: 0; font-family: inherit; cursor: pointer; display: inline-flex; align-items: center; gap: 4px; width: 100%;">
            Router Paths <i data-lucide="chevron-down" style="width: 14px; height: 14px; color: var(--text-primary);"></i>
          </button>
          <div id="router-paths-content" class="accordion-content">
            <ul class="drawer-list" style="margin-top: 8px;">
  ...
  renderSourceCode(metadata) {
    ...
    if (metadata.class_signature) {
      ...
      classSection = `
        <div class="accordion-section expanded">
          <button type="button" class="accordion-subheader accordion-title" aria-expanded="true" aria-controls="class-content" style="border: none; width: 100%; text-align: left; font-family: inherit;">Class</button>
          <div id="class-content" class="accordion-content">
            <div class="drawer-section">
    ...
    return `
      <div class="accordion-section expanded">
        <button type="button" class="accordion-header" aria-expanded="true" aria-controls="source-code-content" style="border: none; width: 100%; text-align: left; font-family: inherit;">
        ...
        <div id="source-code-content" class="accordion-content">
          <div class="drawer-section">
            <div class="code-block-container">
              <button type="button" class="code-copy-button" aria-label="Copy code" data-code="${codeToHighlight.replace(/"/g, "&quot;")}">
  ...
  attachContentEventListeners(nodeName) {
    ...
    const accordionHeaders =
      this.elements.content.querySelectorAll(".accordion-header, .accordion-subheader, .condition-label, .router-paths-title");
    accordionHeaders.forEach((header) => {
      header.addEventListener("click", () => {
        const accordionSection = header.closest(".accordion-section") || header.parentElement;
  ...
  close() {
    ...
    accordions.forEach(accordion => {
      ...
      const header = accordion.querySelector(".accordion-header, .accordion-subheader, .condition-label, .router-paths-title");
      if (header) {
